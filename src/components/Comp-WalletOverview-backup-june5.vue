...
export default {
            ...
            dataLabels: {
              ...
              value: {
                ...
                formatter: function (val, w) {
                  ...
                  console.log('RadialBar Value:', val); // Debugging log
              ...
              total: {
                ...
                formatter: (w) => {     
                  const total = w.globals.seriesTotals.reduce((a, b) => a + b, 0); 
                  console.log('RadialBar Total:', total); 
                  return (this.Overview_Data["Fortification perc_and_count"]["fortified_perc_total"]).toFixed(1) + '%';
                }
              }
            },
            startAngle: -125,
  ...
  computed: {
    ...
    chartSeriesFortification() {
      if (!this.Overview_Data || !this.Overview_Data["Fortification perc_and_count"]) {
        console.log("No data available");
        return [0, 0, 0, 0];
      }

      const seriesData = [
        this.Overview_Data["Fortification perc_and_count"]["fortified_perc_pred"] || 0,
        this.Overview_Data["Fortification perc_and_count"]["fortified_perc_derp"] || 0,
        this.Overview_Data["Fortification perc_and_count"]["fortified_perc_derpling"] || 0,
        this.Overview_Data["Fortification perc_and_count"]["fortified_perc_derpape"] || 0,
      ];

      console.log("Series Data Fortification:", seriesData);
      return seriesData;
    },
...
.toggle-image.selected {
  transform: scale(1.5); /* Increase size by 33% */
}
</style>
