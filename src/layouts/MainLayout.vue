<template>
  <q-layout view="hHh lpR fFf">
      ...
      <q-toolbar>
        ...
        <q-toolbar-title>
          <div class="q-px-lg q-pt-xl q-mb-lg">
            <!-- <div class="text-h4">STUFFD</div> -->
          </div>
        </q-toolbar-title>

        <q-img src="~/assets/SoMuchFun.png" class="header-image absolute top" />
        <q-img src="~/assets/STUFFD-transp.webp" class="logo"/>
      </q-toolbar>

      <q-tabs align="left">
        <q-route-tab to="/" label="My Wallet" />
        <q-route-tab to="/Project-Performance" label="Project" />
      </q-tabs>
    </q-header>
    ...
    <q-drawer v-model="leftDrawerOpen" show-if-above :width="300" :breakpoint="600">
      ...
      <template v-else>
        <q-list padding class="q-list-project"> 

          <q-item to="/Project-Performance" clickable v-ripple>
            <q-item-section avatar>
              <q-icon name="query_stats" /> <!-- alternative: 'assessment' -->
            </q-item-section>
            <q-item-section> Project Performance </q-item-section>
          </q-item>

          <q-item to="/Mining-Setup" clickable v-ripple>
            <q-item-section avatar>
              <q-icon name="developer_board" />
            </q-item-section>
            <q-item-section> Mining Setup </q-item-section>
          </q-item>
...
export default {
  ...
  setup() {
    ...
    const targetPaths = ["/", "/MyRewards", "/MyOutposts"];

    const isTargetPath = () => targetPaths.includes(route.path);
